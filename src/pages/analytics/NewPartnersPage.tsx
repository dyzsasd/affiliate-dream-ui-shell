...
const NewPartnersPage: React.FC = () => {
  ...
  const getNewPartnerDomains = useCallback(() => {
    ...
    // Filter for new partners only - this assumes the API provides date information
    // Adjust this logic based on actual API response structure
    const newPartnerDomains = partnerInfo.promotypeMix.value
      .filter((item: any) => {
        // Example: filter partners added in the last 30 days
        const joinDate = new Date(item.joinDate || item.createdAt || 0);
    ...
    return [...new Set(newPartnerDomains)]; // Remove duplicates
  }, [advertiserData]);
